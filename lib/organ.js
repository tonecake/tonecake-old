...
var Organ = {
    ...
    set : {
        ...
        soprano : function(frequency){

            if ( !frequency ){ var frequency = 0; }
            Organ.info.soprano.frequency.value = frequency;

        },
        alto : function(frequency){

            if ( !frequency ){ var frequency = 0; }
            Organ.info.alto.frequency.value = frequency;

        },
        tenor : function(frequency){

            if ( !frequency ){ var frequency = 0; }
            Organ.info.tenor.frequency.value = frequency;

        },
        bass : function(frequency){

            if ( !frequency ){ var frequency = 0; }
            Organ.info.bass.frequency.value = frequency;

        },
        volume : function(value, node){

            switch( node ){
                case 'soprano' :
                    Organ.info.gainNode1.gain.value = value;
                    break;
                case 'alto' :
                    Organ.info.gainNode2.gain.value = value;
                    break;
                case 'tenor' :
                    Organ.info.gainNode3.gain.value = value;
                    break;
                case 'bass' :
                    Organ.info.gainNode4.gain.value = value;
                    break;
                default :
                    Organ.info.gainNode1.gain.value = value;
                    Organ.info.gainNode2.gain.value = value;
